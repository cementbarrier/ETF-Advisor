...
### 2026-07-26（GUI 线程安全 + 关闭对话框）

| # | 变更 | 说明 |
|---|------|------|
| 19 | 分析输出栏无内容 | `on_run()` 中取消按钮创建在 `top`（pack 布局）上用 `grid()` 放置，TclError 静默崩溃 | 取消按钮父容器改为 `row_a`，用 `pack(side="left")` 放置；`_log`/`_finish_analysis` 改用 `root.after(0, ...)` 切换主线程 |
| 20 | WM_DELETE_WINDOW 协议 | 协议注册在 `if HAS_TRAY:` 内，pystray 未打进 EXE 时跳过 | 提到条件外，无论托盘是否可用都弹出关闭对话框 |
| 21 | 关闭对话框确认按钮 | 取消/确认按钮外观无区分 | 确认按钮蓝底白字 `bg="#0078D4" fg="white"` |

### 代码清理（2026-07-25）
| # | 变更 | 说明 |
|---|------|------|
| 18 | 移除调试日志 | `data_fetcher.py` 删除 `_write_debug_log()` 函数及全部 4 处调用；移除 `Path` import 和 `_PROJECT_ROOT`；项目根目录清理 build.log 等 9 个构建产物 |
